...
public class BeckerbauerBookAnalytics extends BookAnalytics{
	...
	private String[] allTextWithoutSpaces;
	private String allTextWithoutPunctuation;
...
//			}
			newNextLine = newNextLine.replace(currentLetter + "", "");
			letterCount = 0;
		}
	...
	public double averageWordLength() {
		...
		averageWordLength += totalLength;
		return averageWordLength;
	}
	@Override
	public int sentenceCount() {
		String newNextLine = allText.toLowerCase();
		for(int i = 0; i < newNextLine.length(); i++) {
	...
	public int numberOfDifferentWords() { 
		String newNextLine = nextLine.toLowerCase();
		newNextLine = newNextLine.replaceAll("\\p{Punct}", "");
		ArrayList<String> differentWords = new ArrayList<String>(Arrays.asList(allTextWithoutSpaces));
	...
	public String alphabetizeAllByChar() {
		String newNextLine = allTextWithoutPunctuation;
	...
	public String alphabetizeAllByWord() {
		String[] words = this.allTextWithoutSpaces;
		Arrays.sort(words);
	...
	public String replaceWordAndPreserveCase(String aLine, String wordOne, String wordTwo) {
		...
		newNextLine = newNextLine.replace(firstWordLowerCase, secondWordLowerCase);
		...
		newNextLine = newNextLine.replace(firstWordUpperCase, secondWordUpperCase);
		return newNextLine;
	}
	@Override
	...
	public void printFile() {
		...
		sentenceCount();
		averageWordLength();
		averageWordLength = averageWordLength / withoutSpaces.length;
		averageWordsPerSentance();
		allTextWithoutSpaces = allText.split(" ");
		numberOfDifferentWords();
		toString();
		alphabetizeAllByChar();
		System.out.println(alphabetizeAllByChar);
		alphabetizeAllByWord();
		System.out.println(alphabetizeAllByWord);
		System.out.println(allText);
		write.write(allText); //use a textWriter to output a string to a new file
		write.flush(); //clears the writer for the next use
		//System.out.println("*********" + allText);
	}
	public String replaceAllPunctuation() {
		allTextWithoutPunctuation = allText.replaceAll("\\p{Punct}", "");
		return allTextWithoutPunctuation;
	}
